  ...
  function ($scope, $http, AppConstants, DataService, ModuloService) {
   ...
   // ModuloService.atualizaMenu('Movimentos de cobrança e contábil');
    ...
    this.$onInit = function () {
      ...
      vm.getChargeManagement(dataReferencia, ["TIT", "CTB", "AGI"]);
    ...
    $scope.removerVersao = function (id) {
      if (!id) {
        ...
        swal({
          ...
          if (result.value) {
            $http.delete('http://localhost:56834/' + '/api/GestaoCarga?id=' + id)
    ...
    $scope.mudarVersao = function (objeto) {
      ...
      swal({
        ...
        if (result.value) {
          ...
          $http.put('http://localhost:56834/' + '/api/GestaoCarga', objeto)
            .then(function (resultado) {
              this.data = resultado.data.Data;
              swal(
                'Feito!',
          ...
          if (result.value) {
            ...
            $http.post(AppConstants.API_ROOT + '/api/GestaoCarga?' + 'dataref=' + dados.dataref + '&tipo=' + dados.tipo +
              ...
              .catch(function (objeto) {
                if (objeto.data.ErroModel.ErroMensagem !== undefined) {
                  swal({ title: 'Erro', text: ResData.data.ErroModel.ErroMensagem, type: 'error', confirmButtonText: 'Ok' });
                }
                else if (objeto.data.Message !== undefined) {
